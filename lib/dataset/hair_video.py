...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load mask
        maskpath = os.path.join(self.dataset_path, 'body_matting', f'{imagename}.jpg')
        alpha_image = imread(maskpath) / 255.
        alpha_image = alpha_image[:, :, None] #, -1:]
        if self.white_bg:
        ...
        if True:
            ...
            if os.path.exists(os.path.join(self.dataset_path, 'body_landmark2d', f'{imagename}.txt')):
                ...
                data['lmk'] = lmk
            ## load segmentation
            mask_path = os.path.join(self.dataset_path, 'body_matting', f'{imagename}.jpg')
            alpha_image = imread(maskpath) / 255.
