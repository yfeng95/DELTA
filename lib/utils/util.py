...
def copy_state_dict(cur_state_dict, pre_state_dict, prefix='', load_name=None, skip_name=None):
    def _get_params(key):
        key = prefix + key
...
def sample_from_2d(image, coords):
    """ Sample from a 2D image using coordinates.

    Args:
        image (tensor): [B, C, H, W] image tensor
        coords (tensor): [H, W]
    """
