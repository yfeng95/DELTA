...
def generate_image_rays(H, W, near, far):
    """ Generates rays for an image of size (H, W).
    the ray origin is at (0, 0, 0) and the ray direction is normalized, directing to z=1
    the ray is nothing about camera, assume everything to render is already in the image space (aligned with the image)
    Args:
        H (int): image height
        W (int): image width
    ...
    Returns:
        rays (tensor): rays of shape (H, W, 7), center, direction, near, far
    """
    height, width = H, W
    Y, X = torch.meshgrid(
        torch.arange(height, dtype=torch.float32) / height * 2. - 1,
        torch.arange(width, dtype=torch.float32) / width * 2. - 1,
    )
    Z = torch.zeros_like(X) * near
    center = torch.stack((X, Y, Z), dim=-1)[None, ...]
    direction = torch.tensor([0, 0, 1.])[None, None, None, :].repeat(1, height, width, 1)
...
def fancy_integration(rgb_sigma, z_vals, noise_std=0.5, last_back=False, white_back=False, clamp_mode='softplus', fill_mode=None):
    ...
    return rgb_final, depth_final, weights.squeeze(-1)
...
def get_initial_rays_trig(n, num_steps, device, fov, resolution, ray_start, ray_end):
    ...
    # rays_d_cam = torch.stack([x, y, z], -1)
...
def perturb_points(points, z_vals, ray_directions, device):
    ...
    z_vals[:,:,1:-1,:] = z_vals[:,:,1:-1,:] + offset[:,:,1:-1,:]
    points[:,:,1:-1,:] = points[:,:,1:-1,:] + offset[:,:,1:-1,:] * ray_directions.unsqueeze(2)
...
def transform_sampled_points(points, z_vals, ray_directions, device, h_stddev=1, v_stddev=1, h_mean=math.pi * 0.5, v_mean=math.pi * 0.5, mode='normal'):
    ...
    return points, z_vals, transformed_ray_directions, transformed_ray_origins, pitch, yaw
