...
class GeoSIREN(nn.Module):
    """Primary SIREN  architecture used in pi-GAN generators."""

    def __init__(self, input_dim=2, z_dim=100, hidden_dim=256, output_dim=1, last_op=None, scale=1.):
        super().__init__()
        self.input_dim = input_dim
        self.z_dim = z_dim
        self.hidden_dim = hidden_dim
    ...
    def forward(self, input, z=None, **kwargs):
        if z is None:
            z = torch.ones(input.shape[0], self.z_dim, device=input.device)
        frequencies, phase_shifts = self.mapping_network(z)
    ...
    def forward_with_frequencies_phase_shifts(self, input, frequencies, phase_shifts, **kwargs):
        ...
        if self.last_op is not None:
            sigma = self.last_op(sigma)
        sigma = sigma*self.scale

        return sigma #torch.cat([sigma], dim=-1)


class ParamNet(nn.Module):
    """direclty optimize parameters"""

    def __init__(self, size, init_way=None, device=None, last_op=None, scale=1.):
        super().__init__()
    ...
    def forward(self, x=None, y=None): # x, y for placeholder
        param = self.last_op(self.param)*self.scale
        batch_size = x.shape[0]
        param = param.expand(batch_size, -1, -1) 
        return param


''' Texture network from neural head avatar 
'''
